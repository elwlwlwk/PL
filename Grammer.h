...
public:
	std::regex get_operator_regex() { return std::regex("(equ|add|sub|mul|div|pow|nrt|exp|ln|sin|cos|int|float|is|gt|lt|if|print|reset|type|tostring|tovalue)"); };
	std::regex get_integer_regex() { return std::regex("^-?[0-9]+$"); };
	std::regex get_float_regex() { return std::regex("^-?[0-9]+\\.[0-9]+$"); };
	std::regex get_string_regex() { return std::regex("\\\".+\\\""); };
